...
@property (strong , nonatomic) UILabel *numberLabel;//倒计时
...
- (void)viewDidLoad {
    ...
    [self numberLabel];
    _timeCount = 3;
    _timer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(daoshu:) userInfo:nil repeats:YES];
...
- (void)onCompleted:(IFlySpeechError *) error{
    ...
    if (_timeCount == 0) {
        ...
        [UIView animateWithDuration:1 animations:^{
            for (int i = 0; i<100; i++) {
                _numberLabel.font = [LCFont systemFontOfSize:i];
            }
        }];
    }
